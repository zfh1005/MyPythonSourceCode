...
struct _intblock {
    struct _intblock *next;/*_intblock类型结构体指针*/
    PyIntObject objects[N_INTOBJECTS];/*PyIntObject 类型的一维数组*/
};

typedef struct _intblock PyIntBlock;/*_intblock 结构体*/

static PyIntBlock *block_list = NULL;/*_intblock 结构体指针,用来表示当前申请的内存块列表*/
static PyIntObject *free_list = NULL;/*_intblock 结构体指针,用来表示当前空闲的内存块列表*/
...
fill_free_list(void)
{
    ...
    /*链接新申请的内存块到block list 列表中*/
	((PyIntBlock *)p)->next = block_list;
    block_list = (PyIntBlock *)p;
    /* Link the int objects together, from rear to front, then return
       the address of the last int object in the block. */
    /*将PyIntBlock中的PyIntObject数组--objects转变成单向列表*/
...
#define NSMALLPOSINTS           257/*小整数内存池的上限(257)*/
#endif
#ifndef NSMALLNEGINTS
#define NSMALLNEGINTS           5/*小整数内存池的下限(-5)*/
#endif
/*
使用小整数对象的内存池
*/
...
#endif
    if (free_list == NULL) {
		/*如果申请的大数据内存块剩余列表为空,将不能再次申请新的整型数据*/
...
/*
PyIntObject对象的析构函数
将引用计数变为0后的PyIntObject对象内存变为free_list_block
*/
static void
int_dealloc(PyIntObject *v)
{
	/*
	判断销毁的事件是否是在销毁一个PyIntObject对象是发生的
	如果是,将当前的内存块添加到free_list_block中
	*/
    if (PyInt_CheckExact(v)) {
        Py_TYPE(v) = (struct _typeobject *)free_list;
        free_list = v;
    }
	/*销毁对象来源于整数派生类时,只简单的调用tp_free函数*/
    else
...
PyTypeObject PyInt_Type = {
    PyVarObject_HEAD_INIT(&PyType_Type, 0)/*Python对象的共有部分*/
    "int",
    sizeof(PyIntObject),
    0,
